...
控制参数配置：
5. 写入 UART_LCR 以设置数据宽度、停止位、奇偶校验方式，并写 UART_LCR[DLAB] = 0，恢复正常模式
6. 写入 UART_FCR 寄存器设置 FIFO 触发条件并启用 FIFO

中断配置：

7. 设置 UART_IER[ERBFI] = 1 以打开接收有效中断
...
1. 检查 UART_LSR[TFE] 以检查发送 FIFO 是否为空。若 UART_LSR[TFE] = 0，则表明发送 FIFO 中的仍有数据
