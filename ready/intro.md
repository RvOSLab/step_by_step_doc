...
RISC-V 标准还规定了一套 **Supervisor 二进制接口（SBI）**，它运行于 M 模式，掌控着整台计算机，你可以认为它是 RISC-V 上的 BIOS（BIOS又可以看作是最底层的直接接触硬件的一个基本操作系统），它提供了一些环境调用的支持。一般来说，操作系统都不会直接管理硬件资源，而是会通过环境调用来让 SBI 协助管理。
...
SBI 只是一个规范，根据这个规范，不同的厂家（或程序员）可以编写不同的 SBI 程序，一个完整的符合 SBI 规范的程序称为 SBI 实现（下面简称 SBI ），RISC-V 官方做了一个 SBI 实现，叫 OpenSBI，此外还有其他的 SBI 实现，如 BBL、RustSBI 等。SBI 是一个刷写在主板上的一个程序。

如果仍然不明白 SBI 是什么，可以做如下理解：我们拿 OpenSBI 来举例，你可以把它看作是一个比较简陋的操作系统，运行在 M 态（M 模式），它的作用就是承托起传统意义上的存放于硬盘的操作系统（运行在 S 态），这时，传统意义上的操作系统相对于 OpenSBI 来说就是一个软件，这个软件可以通过请求环境调用来让 SBI 协助管理实现一些自己无法完成或没有权限完成的功能。而传统意义上的软件是运行在传统意义上的操作系统之上的，在 RISC-V 中，它就是运行在 U 态的普通程序。
